...
export function SoldByBlock({ sellerName = 'CACAO', avgRating = 0, reviewCount = 0, productCount = 0 }: SoldByBlockProps) {
  return (
    <div className="flex items-center gap-3 p-4 my-4 bg-[#1C2021] border border-[#35383C] rounded-xl">
      <div className="w-10 h-10 rounded-full bg-[#2A2D31] text-white font-semibold flex items-center justify-center flex-shrink-0">
        {sellerName.charAt(0).toUpperCase()}
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-sm text-[#EEF2F7] flex items-center gap-1.5">
          <span className="font-semibold truncate">{sellerName}</span>
          <ShieldCheck size={14} className="text-[#FDC700] flex-shrink-0" />
        </p>
          ...
          {reviewCount > 0 && (
            <>
              ...
              <span className="inline-flex items-center gap-0.5 align-middle">
                <Star size={11} className="fill-[#FDC700] text-[#FDC700]" />
