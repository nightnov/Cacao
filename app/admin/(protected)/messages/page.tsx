...
export default function AdminMessages() {
  ...
  return (
    <div>
      <h1 className="font-serif font-semibold text-4xl text-ink mb-8">Messages</h1>

      {loading ? (
        <div className="bg-bg-panel rounded-lg border border-border p-12 text-center">
          <p className="text-ink-dim">Chargement...</p>
        </div>
      ) : conversations.length === 0 ? (
        <div className="bg-bg-panel rounded-lg border border-border p-12 text-center">
          <p className="text-ink-dim">Aucun message pour l&apos;instant</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-bg-panel rounded-lg border border-border overflow-hidden" style={{ height: '70vh' }}>
          {/* Conversation list */}
          <div className="md:col-span-1 border-r border-border overflow-y-auto">
            {conversations.map(convo => (
              <button
                ...
                className={`w-full text-left px-4 py-4 border-b border-border hover:bg-bg-raised transition-colors ${
                  selectedUserId === convo.userId ? 'bg-bg-raised' : ''
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-semibold text-ink truncate">{convo.customerName}</span>
                  {convo.unreadCount > 0 && (
                    <span className="bg-gold text-ink-invert text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0">
                      {convo.unreadCount}
                    </span>
                  )}
                </div>
                <p className="text-xs text-ink-dimmer truncate">{convo.lastMessage}</p>
              </button>
            ))}
          </div>
            ...
            {selectedConvo ? (
              <>
                <div className="px-6 py-4 border-b border-border">
                  <p className="font-semibold text-ink">{selectedConvo.customerName}</p>
                  <p className="text-xs text-ink-dimmer">{selectedConvo.email}</p>
                </div>
                  ...
                  {selectedConvo.messages.map(msg => (
                    <div key={msg.id} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[75%] rounded-lg px-4 py-3 ${
                        msg.sender === 'admin' ? 'bg-gold text-ink-invert' : 'bg-bg-raised text-ink'
                      }`}>
                        {msg.product_name && (
                          <p className={`text-xs mb-1 font-semibold ${msg.sender === 'admin' ? 'text-ink-invert/70' : 'text-ink-dimmer'}`}>
                            📦 {msg.product_name}
                          </p>
                        )}
                        <p className="text-sm whitespace-pre-wrap">{msg.body}</p>
                        <p className={`text-xs mt-1 ${msg.sender === 'admin' ? 'text-ink-invert/60' : 'text-ink-dimmer'}`}>
              ...
              <div className="flex-1 flex items-center justify-center text-ink-dimmer text-sm">
