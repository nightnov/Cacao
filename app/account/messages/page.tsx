...
function MessagesContent() {
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 max-w-2xl mx-auto px-10 py-16 w-full flex flex-col">
        <h1 className="font-serif font-semibold text-4xl mb-8">Messages</h1>

        <div className="bg-white rounded-lg border border-[#E4DDCF] flex flex-col flex-1 min-h-[400px]">
          <div className="flex-1 overflow-y-auto p-6 space-y-4 max-h-[50vh]">
            {loading ? (
              <p className="text-sm text-[#8A8579] text-center">Chargement...</p>
            ) : messages.length === 0 ? (
              <p className="text-sm text-[#8A8579] text-center py-8">
              ...
              messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender === 'customer' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-lg px-4 py-3 ${
                    ...
                    <p className={`text-xs mt-1 ${msg.sender === 'customer' ? 'text-white/70' : 'text-[#8A8579]'}`}>
          ...
          <form onSubmit={handleSend} className="border-t border-[#E4DDCF] p-4 flex gap-3">
            <textarea
              value={body}
