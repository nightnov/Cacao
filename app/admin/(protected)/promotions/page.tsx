...
function statusOf(p: Promotion): { label: string; cls: string } {
  ...
  if (!p.is_active) return { label: 'Désactivée', cls: 'bg-bg-raised text-ink-dim' }
  if (p.max_uses !== null && p.used_count >= p.max_uses)
    return { label: 'Épuisée', cls: 'bg-danger/15 text-danger' }
  if (p.starts_on && today < p.starts_on)
    return { label: 'Programmée', cls: 'bg-gold/15 text-gold' }
  if (p.ends_on && today > p.ends_on)
    return { label: 'Terminée', cls: 'bg-bg-raised text-ink-dim' }
  return { label: 'Active', cls: 'bg-green/15 text-green-bright' }
}
...
export default function AdminPromotions() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className={INPUT}
              />
              <p className="text-[11px] text-ink-dimmer mt-1">0 pour aucune condition.</p>
            </div>
      ...
      <section className={`${CARD} p-5`}>
        <h2 className="font-serif text-lg text-ink">Remise sur gros panier</h2>
        ...
        <div className="bg-gold/10 border border-gold/30 rounded-lg px-3.5 py-2.5 mt-3 text-[12px] text-ink-dim">
        ...
        <p className="text-[12px] text-ink-dimmer mt-3">
        ...
        <button
          onClick={saveVolume}
          disabled={busy}
          className="mt-4 px-5 py-2 bg-gold hover:bg-gold-dim disabled:opacity-50 text-ink-invert rounded-lg font-semibold text-sm"
        >
          {busy ? 'Enregistrement…' : 'Enregistrer'}
        </button>
      </section>
                  ...
                  return (
                    <tr key={p.id} className="border-b border-border last:border-0">
                      <td className="px-4 py-3">
                        <span className="font-mono font-bold text-sm text-ink">{p.code}</span>
                        {p.description && (
                          <p className="text-[11px] text-ink-dimmer">{p.description}</p>
                        )}
                      </td>
                      <td className="px-4 py-3 text-sm text-ink whitespace-nowrap">
                        {describe(p)}
                      </td>
                      <td className="px-4 py-3 text-sm text-ink-dim whitespace-nowrap">
                        {p.min_order_fcfa > 0
                          ? `Dès ${formatAmount(p.min_order_fcfa)} FCFA`
                          : '—'}
                        {p.once_per_customer && (
                          <span className="block text-[11px] text-ink-dimmer">1 fois par client</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-sm text-ink-dim whitespace-nowrap">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        <button
                          ...
                          disabled={busy}
                          className="ml-3 text-sm text-ink-dim hover:underline"
                        >
